...
<div class="bg-white py-3 me-2 rounded-md">
    ...
    <div class="flex gap-2 mt-3 border-b-2 border-black/10 pb-2">
        <a href="{{ route('admin.typeExercice.index') }}"
           class="px-3 py-1 rounded-full border {{ !$categorieSlug ? 'bg-vert text-white border-vert' : 'bg-black/5 border-black/10' }}">
            Tous
        </a>
        @foreach($categories as $cat)
            <a href="{{ route('admin.typeExercice.index', ['categorie' => $cat->slug]) }}"
               class="px-3 py-1 rounded-full border {{ $categorieSlug === $cat->slug ? 'bg-vert text-white border-vert' : 'bg-black/5 border-black/10' }}">
                {{ $cat->nom }}
            </a>
        @endforeach
    </div>
