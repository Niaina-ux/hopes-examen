...
<div class="w-full py-3">
    ...
    <div class="bg-white p-4 rounded-md">
        <h2 class="text-xl font-semibold mb-4">Modifier l'exercice mots croisés</h2>
        ...
        <form id="mc-form" action="{{ route('prof.examen.motscroises.update', [$slug, $examen->id, $motscroises->id]) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label class="block text-base font-medium">Titre</label>
                <input type="text" name="titre" value="{{ old('titre', $motscroises->titre) }}" class="border rounded w-full p-2">
            </div>

            <div class="mb-4">
                <label class="block text-base font-medium">Description (optionnel)</label>
                <textarea name="description" rows="2" class="border rounded w-full p-2">{{ old('description', $motscroises->description) }}</textarea>
            </div>

            <div class="flex gap-4 mb-4">
                <div class="flex-1">
                    <label class="block text-base font-medium">Durée (minutes)</label>
                    <input type="number" name="duree_minutes" value="{{ old('duree_minutes', $motscroises->duree_minutes) }}" min="1" class="border rounded w-full p-2">
                </div>
                <div class="flex-1">
                    <label class="block text-base font-medium">Note totale</label>
                    <input type="number" name="note_totale" value="{{ old('note_totale', $motscroises->note_totale) }}" min="0.1" step="0.1" class="border rounded w-full p-2">
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <div class="flex gap-3 items-end">
                    <div>
                        ...
                        <input type="number" id="grille-largeur" value="{{ $largeur }}" min="2" max="20" class="border rounded p-2 w-24">
                    </div>
                    <div>
                        <label class="block text-xs text-black/50">Hauteur (lignes)</label>
                        <input type="number" id="grille-hauteur" value="{{ $hauteur }}" min="2" max="20" class="border rounded p-2 w-24">
                    </div>
                    <button type="button" id="btn-generer-grille" class="bg-black/5 border rounded-md px-4 py-2">
                        Réinitialiser la grille (vide)
                    </button>
                </div>
                <p class="text-xs text-rouge mt-1">⚠️ « Réinitialiser » effacera tous les mots actuels de la grille ci-dessous.</p>
            </div>
            ...
            <div class="mb-4">
                ...
                <div id="grille-container" class="inline-block border-2 border-black/20"></div>
            </div>

            {{-- Panneau d'ajout de mot --}}
            <div id="panneau-ajout-mot" class="hidden border-2 border-vert rounded-md p-4 mb-4 bg-vert/5">
                ...
                <div class="flex gap-4 mb-3">
                    <div class="flex-1">
                        ...
                        <select id="input-direction" class="border rounded w-full p-2">
                    ...
                    <div class="flex-1">
                        ...
                        <input type="text" id="input-reponse" class="border rounded w-full p-2 uppercase" maxlength="20" placeholder="Ex: PARIS">
                    </div>
                    <div class="flex-1">
                        <label class="block text-xs text-black/50">Points</label>
                        <input type="number" id="input-points" value="1" min="0.1" step="0.1" class="border rounded w-full p-2">
                    </div>
                </div>

                <div class="mb-3">
                    <label class="block text-xs text-black/50">Indice</label>
                    <input type="text" id="input-indice" class="border rounded w-full p-2" placeholder="Ex: Capitale de la France">
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ✅ Mots déjà enregistrés (venant du serveur), à préremplir
    const motsExistants = @json($mots->map(function ($mot) {
        return [
            'indice' => $mot->indice,
            'reponse' => $mot->reponse,
    ...
    // === Générer une grille vide (réinitialisation manuelle) ===
    ...
    // ✅ Charger les mots existants dans la grille au chargement
    function chargerMotsExistants() {
        initGrilleVide();
        mots = [];
        prochainNumero = 1;

        motsExistants.forEach(function (motData) {
            const mot = {
                id: mots.length + 1,
                indice: motData.indice,
                reponse: motData.reponse,
                direction: motData.direction,
                position_x: motData.position_x,
                position_y: motData.position_y,
                numero: motData.numero,
                points: motData.points,
                positions_lettres_visibles: motData.positions_lettres_visibles,
            };
            ...
            mots.push(mot);
            prochainNumero = Math.max(prochainNumero, mot.numero + 1);
        });
    ...
    function afficherGrille() {
        ...
        for (let y = 0; y < hauteur; y++) {
            ...
            for (let x = 0; x < largeur; x++) {
                ...
                let bgClass = 'bg-black/5 hover:bg-vert/10';
                if (cellule.lettre) {
    ...
    document.getElementById('btn-generer-grille').addEventListener('click', function () {
        if (confirm('Voulez-vous vraiment réinitialiser la grille ? Tous les mots seront effacés.')) {
            genererGrilleVide();
        }
    });

    // ✅ Charge les mots existants au démarrage (au lieu d'une grille vide)
    chargerMotsExistants();
});
</script>
@endsection