@extends('layouts.prof-layouts.proflayoutshead')
@section('contenue-prof')
<div class="bg-white py-3 rounded-md me-2">
    <div class="flex gap-3 items-center my-2">
        <a href="" class="">
            Examen /
        </a>
        <span class="font-semibold">Creation</span>
    </div>
    <h2 class="text-xl font-semibold mb-4 text-vert">Ajouter une question — {{ $text->titre }}</h2>
    @if($errors->any())
        <div class="bg-red-100 text-red-700 p-3 rounded mb-4">
            <ul>
                @foreach($errors->all() as $error)
                    <li>{{ $error }}</li>
                @endforeach
            </ul>
        </div>
    @endif

    <div class="bg-black/5 p-3 rounded-md mb-4 text-sm text-black/60 whitespace-pre-line">
        <p class="text-sm">{{ \Illuminate\Support\Str::limit($text->texte, 150) }}</p>
    </div>
    @if(session('success'))
        <div id="success-alert" class="bg-green-100/50 text-green-700 px-4 py-2 rounded-md my-2 flex justify-between items-center">
            <span>{{ session('success') }}</span>
            <button type="button" onclick="document.getElementById('success-alert').remove()">
                <i class="fa-solid fa-xmark"></i>
            </button>
        </div>
    @endif
    <form action="{{ route('prof.examen.text.question.store', [$slug, $examen->id, $text->id]) }}" method="POST">
        @csrf

        <div class="mb-2">
            <label class="block text-sm font-medium">Énoncé</label>
            <textarea 
                name="enonce" 
                rows="2" 
                class="border border-black/20 rounded w-full p-2" 
                placeholder="Ex: Quel est le thème principal du texte ?"
                >{{ old('enonce') }}</textarea>
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium">Points</label>
            <input 
                type="number" 
                name="points" 
                value="{{ old('points', 1) }}" 
                min="0.1" 
                step="0.1" 
                class="border border-black/20 rounded w-32 p-2">
        </div>

        <button type="submit" class="bg-rouge text-white px-4 py-2 mt-2 rounded">Enregistrer</button>
    </form>
</div>
@endsection