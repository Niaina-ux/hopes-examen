...
<div class="w-full py-3">
    ...
    <div class="bg-white rounded-md me-2">
        ...
        @endif
        <h2 class="text-xl font-semibold my-2 text-vert">Ajouter un exercice de rédaction</h2>
        <form action="{{ route('prof.examen.redaction.store', [$slug, $examen->id]) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="titre" value="{{ old('titre') }}" class="border border-black/20 rounded w-full p-2" placeholder="Ex: Rédaction sur le développement ...">
                @error('titre') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="mb-4">
                <label class="block text-base font-medium">Sujet</label>
                <textarea name="sujet" rows="3" class="border border-black/20 rounded w-full p-2" placeholder="Ex: Développez ce sujet et répondez .">{{ old('sujet') }}</textarea>
            ...
            <div class="mb-4">
                ...
                <textarea name="instruction" rows="3" class="border border-black/20 rounded w-full p-2" placeholder="Consignes supplémentaires...">{{ old('instruction') }}</textarea>
            ...
            <div class="flex gap-4 mb-4 w-[10cm]">
                <div class="flex-1 hidden">
                    ...
                    <input type="number" name="nombre_mots_min" value="{{ old('nombre_mots_min') }}" min="1" class="border border-black/20 rounded w-full p-2 " placeholder="Ex: 100">
                ...
                <div class="flex-1">
                    ...
                    <input type="number" name="nombre_mots_max" value="{{ old('nombre_mots_max') }}" min="1" class="border border-black/20 rounded w-full p-2" placeholder="Ex: 250">
                ...
                <div class="flex-1">
                    ...
                    <input type="number" name="note_totale" value="{{ old('note_totale') }}" min="0.1" step="0.1" class="border border-black/20 rounded w-full p-2">
                    @error('note_totale') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
            </div>
            <button type="submit" id="submit-btn" class="bg-rouge text-white px-4 py-2 rounded mt-2">Enregistrer</button>
        </form>
    </div>
</div>
