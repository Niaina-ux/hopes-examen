...
<div class="bg-white p-4 rounded-md">
    ...
    <form action="{{ route('prof.examen.code.update', [$slug, $examen->id, $code->id]) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label class="block text-sm font-medium">Titre du QCM</label>
            <input type="text" name="titre" value="{{ old('titre', $code->titre) }}" class="border rounded w-full p-2">
            @error('titre') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium">Description</label>
            <textarea name="description" rows="3" class="border rounded w-full p-2">{{ old('description', $code->description) }}</textarea>
        ...
        <div class="flex gap-4 mb-4">
            <div class="flex-1">
                ...
                <input type="number" name="note_totale" value="{{ old('note_totale', $code->note_totale) }}" class="border rounded w-full p-2">
