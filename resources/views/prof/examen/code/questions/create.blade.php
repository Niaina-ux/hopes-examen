...
<div class="w-full py-3">
    ...
    <div class="bg-white p-4 rounded-md">
        ...
        @endif
        <h2 class="text-xl font-semibold mb-4">Ajouter un exercice de code — {{ $code->titre }}</h2>
        <form action="{{ route('prof.examen.code.question.store', [$slug, $examen->id, $code->id]) }}" method="POST">
            @csrf

            <div class="mb-4">
                <label class="block text-base font-medium">Instruction</label>
                <textarea name="instruction" rows="3" class="border rounded w-full p-2" placeholder="Ex: Écrivez une fonction qui calcule la somme de deux nombres.">{{ old('instruction') }}</textarea>
            ...
            <div class="mb-4">
                ...
                <select name="langage" class="border rounded w-full p-2">
                    <option value="ensemble" {{ old('langage') == 'ensemble' ? 'selected' : '' }}>Ensemble</option>
                    ...
                    <option value="laravel" {{ old('langage') == 'laravel' ? 'selected' : '' }}>Laravel</option>
                </select>
