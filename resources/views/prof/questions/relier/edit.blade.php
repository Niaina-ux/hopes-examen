...
<div class=" py-3 ">
    ...
    <form action="{{ route('prof.question.relier.update', [$slug, $relier->id]) }}"
        method="POST"
        ...
        <div class="mb-1">
            ...
            <textarea
                ...
                rows="3"
                class="border border-black/20 bg-white/90 formulaire rounded w-full p-2
                dark:bg-white/2 dark:border-white/10">{{ old('description', $relier->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm">{{ $message }}</p>
            @enderror
        </div>
        ...
        <button type="submit"
            class="bg-rouge text-white px-5 py-2 mt-5 rounded-full">
