...
class ProfCorrigeExamenFichierController extends Controller
{
    ...
    public function showtache(string $slug, string $examenId, string $studentId)
    {
        ...
        return view('prof.student.planexamencorrige.fichiers', compact(
            'slug',
            'examen',
            'student',
            'fichiers',
    ...
    public function storeAnnotation(Request $request, Fichier $fichier)
    {
        $reponseIds = array_keys($request->input('reponses', []));
        $pointsParReponse = FichierReponse::whereIn('id', $reponseIds)
            ->with('fichierQuestion') 
            ->get()
            ->mapWithKeys(fn($r) => [$r->id => $r->fichierQuestion->points]);
        ...
        DB::transaction(function () use ($validated) {
            foreach ($validated['reponses'] as $reponseId => $data) {
                $reponse = FichierReponse::find($reponseId);
                if (!$reponse) {
                    continue;
                }
